...
const rules = [
    ...
    { id: "sym", label: "One special symbol (!@#$%^&*…)", test: (p: string) => /[^A-Za-z0-9]/.test(p) },
];
...
function strengthLabel(s: number) {
    ...
    if (s <= 2) return { label: "Fair", color: "#f97316" };
    if (s <= 3) return { label: "Good", color: "#eab308" };
    if (s <= 4) return { label: "Strong", color: "#22c55e" };
    return { label: "Excellent", color: "#10b981" };
}


/* ─── Simple Bot Check (math CAPTCHA) ─── */
function useCaptcha() {
    ...
    const check = () => {
        ...
        else { setAnswer(""); setA(Math.floor(Math.random() * 9) + 1); setB(Math.floor(Math.random() * 9) + 1); }
    };
...
export default function SignUpPage() {
    ...
    const canSubmit = name && email && allRulesMet && passwordsMatch && verified;
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-white to-orange-50 dark:from-black dark:via-black dark:to-black">
                <div className="text-center p-12 rounded-3xl bg-white dark:bg-[#0d1526] border border-amber-200 dark:border-amber-800 shadow-2xl max-w-md w-full mx-4">
                    ...
                    <p className="text-sm text-amber-500 font-semibold">Check your email to verify your account.</p>
                    <button onClick={() => setSubmitted(false)} className="mt-8 px-8 py-3 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 text-white font-bold text-sm hover:scale-105 transition-transform">
                        Back to Sign Up
                    </button>
                </div>
            </div>
    ...
    return (
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        .fade-up-8 { animation-delay: 0.60s; }
            ...
            <div className="min-h-screen flex bg-gradient-to-br from-amber-50/40 via-white to-orange-50/30 dark:from-[#030712] dark:via-[#0a0f1e] dark:to-[#030712]">
                    ...
                    <div className="w-full max-w-md">
                        ...
                        <div className="fade-up fade-up-2 mb-8">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                Already have one?{" "}
                                <a href="/signin" className="text-amber-500 hover:text-amber-600 font-semibold transition-colors">
                                    Sign in →
                                </a>
                            </p>
                        </div>
                ...
                ">
                            ...
                            <div className="fade-up fade-up-3 flex items-center gap-3">
                                ...
                                <span className="text-xs text-slate-400 dark:text-slate-600 font-medium">or register with email</span>
                                <div className="flex-1 h-px bg-slate-200 dark:bg-slate-800" />
                            </div>
                                                    ...
                                                    <div className="flex-1 flex gap-1">
                                                        {[1, 2, 3, 4, 5].map(i => (
                                                            <div key={i} className="flex-1 h-1.5 rounded-full transition-all duration-300"
                                                                style={{ background: i <= score ? sColor : "#e2e8f0" }} />
                                                        ))}
                                                    </div>
                                                    <span className="text-[11px] font-bold" style={{ color: sColor }}>{sLabel}</span>
                                                </div>
                                                {/* Rules checklist */}
                                                    ...
                                                    {rules.map(r => {
                                                        ...
                                                        return (
                                                            <div key={r.id} className="flex items-center gap-2">
                                                                <span className={`text-xs transition-all duration-200 ${ok ? "text-emerald-500" : "text-slate-300 dark:text-slate-600"}`}>
                                                                    {ok ? "✓" : "○"}
                                                                </span>
                                                                <span className={`text-[11px] transition-colors duration-200 ${ok ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-slate-400 dark:text-slate-600"}`}>
                                                                    {r.label}
                                                                </span>
                                                            </div>
                                                        );
                ...
                ">
                      ...
                      ">
                        ...
                        "
                                            ...
                                            <button type="button" onClick={check}
                                                className="
                          px-4 py-2.5 rounded-lg text-xs font-bold
                        ...
                        ">
                                    ...
                                    <p className="text-[10px] text-slate-400 dark:text-slate-600 mt-2">
                                        Simple math challenge to keep bots out — no third-party tracking.
                                    </p>
                                </div>
                            </div>
                  ...
                  `}
                                >
                                    {canSubmit ? "Create Account →" : "Complete all fields above"}
                                </button>
                            </div>
